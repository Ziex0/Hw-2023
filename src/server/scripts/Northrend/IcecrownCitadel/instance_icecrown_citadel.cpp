    ...
    public:
        ...
        struct instance_icecrown_citadel_InstanceMapScript : public InstanceScript
        {
            ...
            void OnCreatureCreate(Creature* creature)
            {
                if (!TeamInInstance)
                {
                    ...
                    if (!players.isEmpty())
                        if (Player* player = players.begin()->getSource())
                            TeamInInstance = player->GetTeam();
						//PrepareGunshipEvent(Player* player); // Spawn Gunship Event
                }
                ...
                switch (creature->GetEntry())
                {
                    ...
                    case NPC_SE_HIGH_OVERLORD_SAURFANG:
                        if (TeamInInstance == ALLIANCE)
                            creature->UpdateEntry(NPC_SE_MURADIN_BRONZEBEARD, creature->GetGUID());
                        // no break;
                ...
                switch (eventId)
                {
                    ...
                    case EVENT_PLAYERS_GUNSHIP_COMBAT:
                        if (GameObject* go = source->ToGameObject());
                            //if (Transport* IsTransport = !go->IsTransport())
                                //IsTransport->EnableMovement(false);
                        break;
                    case EVENT_PLAYERS_GUNSHIP_SAURFANG:
                    {
                        ...
                        if (GameObject* go = source->ToGameObject());
                            //if (Transport* transport = go->IsTransport());
            ...
            /*void PrepareGunshipEvent(Player* player)
            {
                ...
                if(TeamInInstance == HORDE)
                {
                    if(t = sMapMgr->LoadTransports(instance, GO_THE_SKYBREAKER_HORDE_ICC, 77800))
                    {
                        t->AddNPCPassengerInInstance(NPC_GB_SKYBREAKER, -17.156807f, -1.633260f, 20.81273f, 4.52672f);
                        t->AddNPCPassengerInInstance(NPC_GB_MURADIN_BRONZEBEARD, 13.51547f, -0.160213f, 20.87252f, 3.10672f);
                        t->AddNPCPassengerInInstance(NPC_GB_HIHG_CAPTAIN_JUSTIN_BARTLETT, 42.78902f, -0.010491f, 25.24052f, 3.00672f);
