...
public:
    ...
    ChatCommand* GetCommands() const
    {
...

        {
			...
			{ "changerace",          SEC_VIP_2,  	false, &HandleChangeRaceCommand,      	"", NULL },
			{ "changefaction",		 SEC_VIP_2,  	false, &HandleChangeFactionCommand,		"", NULL },
    ...
    static bool HandleVipWhisperCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (argStr == "off")
        {
            ...
            handler->SendSysMessage(LANG_NOT_WHISPER);
            return true;
        }
	...
	static bool HandleHealCommand(ChatHandler * handler, char const* args)
{
	...
	static bool HandleVipMallCommand(ChatHandler* handler, const char* args)
    {
		...
		me->TeleportTo(530, -248.5119f, 934.2169f, 85.1288f, 1.5998f);
