...
static std::string website = "webxdx778"; 
static std::string times = "wow_char";    // FusionCMS database name
...
static uint32 threeMonthsVipPrice = 14;            // VIP account price for 3 months (in donation points) (Default: 50 donate points)
static uint32 nineMonthsVipPrice = 23;            // VIP account price for 3 months (in donation points) (Default: 50 donate points)
static uint32 permaMonthsVipPrice = 20;
...
public:
	...
	bool OnGossipHello(Player * pPlayer, Creature * pCreature)
	{
		...
		//if (SelectDPoints(pPlayer) < nineMonthsVipPrice)
			//pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_INTERACT_1, "|TInterface/icons/Spell_Shadow_Shadowform:25|tPurchase |cffFF0000Premium|r Account for nine months (|cff980000Locked|r / Information 23 Dp", GOSSIP_SENDER_MAIN, 1);
		//else if (SelectDPoints(pPlayer) >= nineMonthsVipPrice)
			//pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_INTERACT_1, "|TInterface/icons/Spell_Shadow_Shadowform:25|tPurchase |cffFF0000Premium|r Account for nine months (|cff009900Unlocked|r / Click to use) 23 Dp", GOSSIP_SENDER_MAIN, 4);
		
		// Purchase VIP Accountpermanent
		//if (SelectDPoints(pPlayer) < permaMonthsVipPrice)
			//pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_INTERACT_1, "|TInterface/icons/Spell_Shadow_Shadowform:25|tPurchase |cffFF0000Premium|r permanent (|cff980000Locked|r / Information) 20 Dp", GOSSIP_SENDER_MAIN, 1);
	...
	bool OnGossipSelect(Player * pPlayer, Creature * pCreature, uint32 sender, uint32 uiAction)
	{
		...
		case 999: // Not enought points
			pPlayer->GetSession()->SendAreaTriggerMessage("Not dont have enought points, Donate more on Www.Heaven-wow.Com");
