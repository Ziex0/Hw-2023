...
{
	int debugLevel = ConfigMgr::GetIntDefault("VASAutoBalance.DebugLevel", 2);
...
public:
	...
	void OnBeforeConfigLoad(bool reload) override
	{
		...
		if (!reload) {
			...
			ConfigMgr::LoadMore(cfg_def_file.c_str());

			ConfigMgr::LoadMore(cfg_file.c_str());
		}
	...
	void SetInitialWorldSettings()
	{
		...
		LoadForcedCreatureIdsFromString(ConfigMgr::GetStringDefault("VASAutoBalance.ForcedID40", ""), 40);
		LoadForcedCreatureIdsFromString(ConfigMgr::GetStringDefault("VASAutoBalance.ForcedID25", ""), 25);
		LoadForcedCreatureIdsFromString(ConfigMgr::GetStringDefault("VASAutoBalance.ForcedID10", ""), 10);
		LoadForcedCreatureIdsFromString(ConfigMgr::GetStringDefault("VASAutoBalance.ForcedID5", ""), 5);
		LoadForcedCreatureIdsFromString(ConfigMgr::GetStringDefault("VASAutoBalance.ForcedID2", ""), 2);
...
public:
	...
	{
		if (ConfigMgr::GetBoolDefault("VAS.Announce", true))
		{
...
public:
	...
	{
		if ((AttackerUnit->GetMap()->IsDungeon() && playerVictim->GetMap()->IsDungeon()) || ConfigMgr::GetIntDefault("VASAutoBalance.DungeonsOnly", 1) < 1)
	...
	{
		if ((caster->GetMap()->IsDungeon() && target->GetMap()->IsDungeon()) || ConfigMgr::GetIntDefault("VASAutoBalance.DungeonsOnly", 1) < 1)
	...
	{
		if (ConfigMgr::GetIntDefault("VASAutoBalance.DungeonsOnly", 1) < 1 || (damageInfo->attacker->GetMap()->IsDungeon() && damageInfo->target->GetMap()->IsDungeon()) || (damageInfo->attacker->GetMap()->IsBattleground() && damageInfo->target->GetMap()->IsBattleground()))
		{
	...
	void CalculateMeleeDamage(Unit *playerVictim, uint32 damage, CalcDamageInfo *damageInfo, WeaponAttackType attackType)
	{
		...
		if ((ConfigMgr::GetIntDefault("VASAutoBalance.DungeonsOnly", 1) < 1 || (damageInfo->attacker->GetMap()->IsDungeon() && damageInfo->target->GetMap()->IsDungeon()) || (damageInfo->attacker->GetMap()->IsBattleground() && damageInfo->target->GetMap()->IsBattleground())) && (damageInfo->attacker->GetTypeId() != TYPEID_PLAYER))
...
public:
	...
	{
		if (ConfigMgr::GetIntDefault("VASAutoBalance.PlayerChangeNotify", 1) > 0)
		{
...
public:
	...
	void Creature_SelectLevel(const CreatureTemplate *creatureTemplate, Creature* creature)
	{
		...
		if (creature->GetMap()->IsDungeon() || ConfigMgr::GetIntDefault("VASAutoBalance.DungeonsOnly", 1) < 1)
		{
	...
	void OnAllCreatureUpdate(Creature* creature, uint32 diff)
	{
		...
		{
			if (creature->GetMap()->IsDungeon() || creature->GetMap()->IsBattleground() || ConfigMgr::GetIntDefault("VASAutoBalance.DungeonsOnly", 1) < 1)
				ModifyCreatureAttributes(creature);
			CreatureInfo[creature->GetGUID()].instancePlayerCount = creature->GetMap()->GetPlayersCountExceptGMs() + PlayerCountDifficultyOffset;
		}
	}
	...
	{
		if (((creature->IsHunterPet() || creature->IsPet() || creature->IsSummon()) && creature->IsControlledByPlayer()) || (creature->GetMap()->IsDungeon() && ConfigMgr::GetIntDefault("VASAutoBalance.Instances", 1) < 1) || creature->GetMap()->GetPlayersCountExceptGMs() <= 0)
		{
			return;
		}
		...
		if ((creature->GetMapId() == 0 || creature->GetMapId() == 1 || creature->GetMapId() == 530) && (creature->isElite() || creature->isWorldBoss()))  // specific to World Bosses and elites in those Maps, this is going to use the entry XPlayer in place of instancePlayerCount.
		{
			if (baseHealth > 800000) {
				healthMultiplier = (tanh((ConfigMgr::GetFloatDefault("VASAutoBalance.numPlayer", 1.0f) - 5.0f) / 1.5f) + 1.0f) / 2.0f;

			}
			else {
				healthMultiplier = (tanh((ConfigMgr::GetFloatDefault("VASAutoBalance.numPlayer", 1.0f) - 2.2f) / 1.5f) + 1.0f) / 2.0f; // Assuming a 5 man configuration, as World Bosses have been relatively retired since BC so unless the boss has some substantial baseHealth
			}

		}

		// Ensure that the healthMultiplier is not lower than the configuration specified value. -- This may be Deprecated later.
		if (healthMultiplier <= ConfigMgr::GetFloatDefault("VASAutoBalance.MinHPModifier", 0.1f))
		{
			healthMultiplier = ConfigMgr::GetFloatDefault("VASAutoBalance.MinHPModifier", 0.1f);
		}
		...
		if (damageMultiplier <= ConfigMgr::GetFloatDefault("VASAutoBalance.MinDamageModifier", 0.1f))
		{
			damageMultiplier = ConfigMgr::GetFloatDefault("VASAutoBalance.MinDamageModifier", 0.1f);
		}
